...
вывод `ss -tlnp | grep :80`, `systemctl status nginx`
...
вывод `curl -v http://IP-ВАШЕГО-VPS`
...
вывод `cat /etc/nginx/sites-available/default`:
...
Запрос к локальному резолверу для получения списка корневых серверов.<br>
`. 254743 IN NS a.root-servers.net.`<br>
`...`<br>
`;; Received 811 bytes from 127.0.0.53#53(127.0.0.53)`

### Шаг 2. Получение данных о зоне TLD (.net)

Корневой сервер (a.root-servers.net) отправляет нас к серверам, отвечающим за зону .net. <br>
...
Сервер зоны .net (d.gtld-servers.net) сообщает адреса серверов, которые знают всё про домен ts.net. <br>
`ts.net. 172800 IN NS ns1.dnsimple.com.` <br>
`...` <br>
`;; Received 573 bytes from 192.31.80.30#53(d.gtld-servers.net)`

### Шаг 4. Финальная A-запись (usoltsev.puma-mintaka.ts.net)

Авторитарный сервер ns1.dnsimple.com выдает итоговый публичный IP-адрес сервера. <br>
`usoltsev.puma-mintaka.ts.net. 300 IN A 185.40.234.37`<br>
